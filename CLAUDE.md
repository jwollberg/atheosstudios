...
- **Copy is grounded in the real game.** Grave Error facts come from
  https://www.atheosstudios.com/grave-error — top-down zombie survival, HACK-nanobot
  premise (player is a "Null"), **single-player AND multiplayer**, **PC only** (no
  console), Early Access targeted **Summer 2026**, philosophy "realism first."
  Don't reinvent lore; re-derive from that page (or Josh) when updating.
- **Josh removed from the mockup:** the "Play Grave Error" button (game isn't out —
  ...
  ("Project II" / "Reserved Slot"), and the social-links grid (no socials yet).
- **Contact = info@atheosstudios.com** (email only, no form, no socials).
